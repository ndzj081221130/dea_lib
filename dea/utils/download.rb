class Download
  ...
  def download!(&blk)
	...
	logger.info("--- by zhang: in download method, destination_dir=#{destination_dir}")
      ...
      cleanup.call do
        ...
        if http_status == 200
          ...
          if !sha1_expected || sha1_expected == sha1_actual
            logger.info("--- by zhang : Download succeeded , file_path=#{file.path}")
          ...
          logger.warn(error.message, error.data)
          logger.info("---Today: file_path=#{file.path}")
          blk.call(error)
        end
      end
    end
