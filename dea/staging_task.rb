...
module Dea
  class StagingTask < Task
    ...
    def memory_limit_in_bytes
      #logger.info("zhang: config.minimum = #{config.minimum_staging_memory_mb}")
    ...
    def promise_stage
      Promise.new do |p|
        ...
        logger.info("-3--Staging: #{script}")
        time = staging_timeout + staging_timeout_grace_period
        #logger.info ("-3-- staging time = #{time}")
    ...
    def promise_staging_info
      Promise.new do |p|
	      logger.debug("-10--  in staging info")
    ...
    def promise_unpack_app
      Promise.new do |p|
        logger.info("-1--Unpacking app to #{workspace.warden_unstaged_dir}")
    ...
    def promise_pack_app
      Promise.new do |p|
	      logger.info("-4--packing app to #{workspace.warden_staged_droplet}")
        promise_warden_run(:app, <<-BASH).resolve
      ...
      Promise.new do |p|
        ...
        Download.new(attributes["download_uri"], workspace.workspace_dir, nil, logger).download! do |error, path|
          if error
            ...
            logger.debug(" in app download : Moved droplet to #{workspace.downloaded_droplet_path}")
            p.deliver
          end
        end
      end
    ...
    def promise_log_upload_started
      Promise.new do |p|
        logger.debug("-6--   in droplet upload")
        promise_warden_run(:app, <<-BASH).resolve
    ...
    def promise_buildpack_cache_upload
      Promise.new do |p|
        Upload.new(workspace.staged_buildpack_cache_path, attributes["buildpack_cache_upload_uri"], logger).upload! do |error|
          if error
 	          logger.info("-8--  by: got an error while uploading buildcache")
            p.fail(error)
          else
            logger.info("-8-- Uploaded buildpack cache to #{attributes["buildpack_cache_upload_uri"]}")
            p.deliver
          end
        end
      end
    ...
    def promise_log_upload_finished
      logger.debug("---9---in promise log upload finished")
      Promise.new do |p|
        promise_warden_run(:app, <<-BASH).resolve
          echo "-9----> Uploaded droplet" >> #{workspace.warden_staging_log}
        BASH
        p.deliver
      end
    end
    ...
    def promise_copy_out
      Promise.new do |p|
        logger.info("-5--Copying out to #{workspace.staged_droplet_path}")
...
#
        ...
        logger.debug("zhang:in promise_container_info")
    ...
    def promise_save_buildpack_cache
      Promise.new do |p|
        logger.debug("-8--  in droplet upload")
    ...
    def promise_unpack_buildpack_cache
      Promise.new do |p|
        if File.exists?(workspace.downloaded_buildpack_cache_path)
          logger.info("-2--Unpacking buildpack cache to #{workspace.warden_cache}, log=#{workspace.warden_staging_log}")
    ...
    def resolve_staging
      ...
      logger.info("---zhang in resolve_staging ensure destroy_place 0 staging ")
    ...
    def staging_timeout
      (staging_config["max_staging_duration"] || "18000").to_f
    end
