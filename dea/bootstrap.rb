  ...
  class Bootstrap
    ...
    def create_instance(attributes)
      ...
      instance.on(Instance::Transition.new(:starting, :running)) do
        ...
        send_heartbeat([instance]) # 通知其他实例？

        # Register with router
        router_client.register_instance(instance) # 调用router_client注册instance实例
      end
    ...
    def handle_router_start(message)
      ...
      register_directory_server_v2 #同时，注册direct——server
    end
    ...
    def handle_dea_directed_start(message)
      logger.info "called by who? msg = #{message}"
    ...
    def handle_dea_update(message) # dea update？只是将当前的uri更新了一下？
#       called by who ? what is message
#       如果CF 中提供了dea的update功能，那么我们的动态更新是不是更简单了？？？
      app_id = message.data["droplet"].to_s
      uris = message.data["uris"]
# =>    根据message中的uri消息，更新当前注册表中的uri信息，并且重新向router注册
